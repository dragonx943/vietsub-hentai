...
init -501 screen Day:
    ...
    if daytimenumber == 0:
        add "heurem.webp" pos (12, 10)
    if daytimenumber == 1:
        add "heurea.webp" pos (12, 10)
    if daytimenumber == 2:
        add "heuree.webp" pos (12, 10)
    if daytimenumber == 3:
        add "heuren.webp" pos (12, 10)
    if daytimenumber == 4:
        add "heureln.webp" pos (12, 10)

    imagebutton:
        auto "heure_%s.webp" pos (24, 13)
        action Jump("passtime")
    if daytimenumber == 0:
        text "09:00 Sáng" style "daystyle"
    if daytimenumber == 1:
        text "02:00 Tối" style "daystyle"
    if daytimenumber == 2:
        text "06:00 Tối" style "daystyle"
    if daytimenumber == 3:
        text "10:00 Tối" style "daystyle"
    if daytimenumber == 4:
        text "01:00 Sáng" style "daystyle"

init -1 style daystyle:
    xpos 0.02
    ypos 0.02
    background "fadeb.png"
    size 32
init -1 style daystyle:
    variant "small"
    xpos 0.02
...
init -501 screen quick_menu():
    ...
    if quick_menu:
        ...
        hbox:
            ...
            textbutton _("L.Sử") action ShowMenu('history')
...
init -501 screen navigation():
    ...
    vbox:
        ...
        hbox:
            xoffset -50
            add "icons/patreonlogo.png" pos (0,10) xzoom 0.2 yzoom 0.2
            textbutton _(" Tìm hiểu Game tại Patreon") action OpenURL("https://patreon.com/eroniverse")
        hbox:
            xoffset -50
            add "icons/discord.png" pos (0,5) xzoom 0.2 yzoom 0.2
            textbutton _(" Tham Gia Discord!") action OpenURL("https://discord.gg/zGMjZCj")
        if main_menu:

            hbox:
                xoffset -50
                add "icons/instagram_icon.png" pos (0,5) xzoom 0.2 yzoom 0.2
                textbutton _(" Tham Gia Instagram!") action OpenURL("https://www.instagram.com/erodraw.art")
            hbox:
                xoffset -50
                add "icons/twitter_icon.png" pos (0,5) xzoom 0.2 yzoom 0.2
                textbutton _(" Tham Gia Twitter!") action OpenURL("https://twitter.com/eroniverse")
            hbox:



                textbutton _("Twitter của Người Lồng Tiếng") text_size 23 action OpenURL("https://twitter.com/MidnightDatura")
        if _in_replay:

            textbutton _("Kết Thúc Tua Lại") action EndReplay(confirm=True)
        ...
        if renpy.variant("pc"):
            ...
            textbutton _("Thoát Khỏi Trò Chơi") action Quit(confirm=not main_menu)
...
init -501 screen main_menu():
    ...
    frame:
        ...
        xsize 1000
        has vbox:
            yalign 0.15 xalign -0.1
            pos (0.0,0.0)

        null height 10
        if ispremiumversion == True:
            text "{b}Phiên bản Premium sẽ mở khóa các gợi ý và hoạt ảnh được phát lại.{/b}" size 23 color "#ff3262" outlines [ (2.0, "#1f1717", 1, 1) ]

    if ifbrowser == True:
        $ renpy.music.set_volume(0.0, delay=0, channel='music')
        text "{b}Đây là bản Web của Game,\n nếu Game không được tối ưu hoặc bạn bị mất File Save Game, \nhãy chơi bản PC\n miễn phí ở dưới phần mô tả.{/b}" pos (1900,600) size 25 color "#ff3262" outlines [ (2.0, "#1f1717", 1, 1) ]
    else:
    ...
    if gui.show_name:
        ...
        vbox:
            text "{color=#ff8bcd}[config.name!t]{/color}":
                style "main_menu_title"

            text "{color=#ff8bcd}[config.version]{/color}":
...
init -501 screen about():
    ...
    use game_menu(_("Thông Tin"), scroll="viewport"):
        ...
        vbox:
            ...
            text _("Được tạo bằng {a=https://www.renpy.org/}Ren'Py{/a} [renpy.version_only].\n\n[renpy.license!t]")
...
init -501 screen file_slots(title):
    ...
    default page_name_value = FilePageNameInputValue(pattern=_("Trang {}"), auto=_("Tự Động Lưu"), quick=_("Lưu Nhanh"))
            ...
            grid gui.file_slot_cols gui.file_slot_rows:
                ...
                for i in range(gui.file_slot_cols * gui.file_slot_rows):
                    ...
                    button:
                        ...
                        text FileTime(slot, format=_("{#file_time}%A, %B %d %Y, %H:%M"), empty=_("chỗ save này trống")):
        ...
        vbox:
            ...
            hbox:
                ...
                if renpy.variant("pc"):
                    ...
                    vbox:
                        ...
                        textbutton _("Dạng cửa sổ") action Preference("display", "window")
                        textbutton _("Toàn màn hình") action Preference("display", "fullscreen")

                vbox:
                    style_prefix "radio"
                    label _("Khu vực quay lại")
                ...
                vbox:
                    style_prefix "check"
                    label _("Bỏ qua")
                    textbutton _("Phần chữ chưa nhìn thấy") action Preference("skip", "toggle")
                    textbutton _("Sau khi chọn") action Preference("after choices", "toggle")
            ...
            hbox:
                ...
                vbox:
                    ...
                    label _("Thời gian tự động chuyển tiếp")
                ...
                vbox:
                    ...
                    if config.has_voice:
                        label _("Âm lượng của giọng nói")

                        hbox:
                            bar value Preference("voice volume")

                            if config.sample_voice:
                                textbutton _("Nghe thử") action Play("voice", config.sample_voice)

                    if config.has_music or config.has_sound or config.has_voice:
                        null height gui.pref_spacing

                        textbutton _("Câm nín hết"):
...
init -501 screen history():
    ...
    use game_menu(_("Lịch Sử"), scroll=("vpgrid" if gui.history_height else "viewport"), yinitial=1.0):
        ...
        if not _history_list:
            label _("Lịch sử của đoạn hội thoại hiện đang trống.")
...
init -501 screen keyboard_help():
    ...
    hbox:
        label _("Phím Space")
        text _("Mở đoạn hội thoại khác mà không cần lựa chọn.")

    hbox:
        label _("Các phím UP, DOWN, LEFT, RIGHT")
        text _("Di chuyển giao diện Game.")

    hbox:
        label _("Phím ESC")
        text _("Mở Menu của Game.")

    hbox:
        label _("Phím Ctrl")
        text _("Bỏ qua nhiều đoạn hội thoại nếu nhấn giữ.")

    hbox:
        label _("Phím Tab")
        text _("Bỏ qua đoạn hội thoại nhanh.")
...
init -1 style windowthink:
    variant "small"
    background "gui/textboxth.png"
